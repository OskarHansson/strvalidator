...
plotRatio_gui <- function(env = parent.frame(), savegui = NULL, debug = FALSE, parent = NULL) {
  ...
  f7 <- gframe(
    text = "Plot marker ratios",
    horizontal = TRUE,
    container = gv
  )

  plot_browse_btn <- gbutton(text = "Browse", container = f7)
  tooltip(plot_browse_btn) <- "Activate the console window and use Enter key to change plot"

  plot_all_btn <- gbutton(text = "Plot", container = f7)
        ...
        for (i in seq(along = vecColNames)) {
          if ("Group" %in% names(val_data)) {
            ...
            if (val_titles) {
              ...
              gp <- .applyPlotSettings(
                gp = gp, theme = val_theme, main.title = "Marker ratio",
