...
#'
#' @details
#' This function provides a graphical user interface (GUI) for managing kits, including the ability to
#' import new kits, edit the short and full names of existing kits, or remove kits.
#' The gender marker of each kit is auto-detected but can be manually adjusted.
#' Note that the short name for each kit must be unique.
#'
...
#' @export
#'
#' @importFrom gWidgets2 gwindow ggroup gframe gradio gbutton gedit glabel addSpring visible svalue size focus gmessage
#' @import gWidgets2tcltk
#' @importFrom utils write.table read.delim help

# Core GUI function
manageKits_gui <- function(env = parent.frame(), savegui = NULL, debug = FALSE, parent = NULL) {
  ...
  strings <- update_strings_with_language_file(default_strings, dt_strings$value)
  ...
  main_group <- ggroup(horizontal = FALSE, spacing = 1, use.scrollwindow = FALSE, container = main_window, expand = TRUE)
  ...
  action_selection_frame <- gframe(text = strings$str_frm_action, horizontal = FALSE, spacing = 1, container = main_group)
  kit_option_radio <- gradio(items = c(strings$str_rad_add, strings$str_rad_edit), selected = 1, container = action_selection_frame)
  
  addHandlerChanged(kit_option_radio, handler = function(h, ...) {
    # Get status of add_kit flag
    add_kit <- svalue(kit_option_radio, index = TRUE) == 1
    ...
    if (add_kit) {
      ...
      # Disable these widgets
      enabled(file_path_frame) <- FALSE # Load kit file options are disabled since loading happens automatically
  ...
  # Kit File Path
  file_path_frame <- gframe(text = strings$str_frm_file, horizontal = TRUE, spacing = 1, container = main_group)
  file_path_edit <- gedit(text = file_path, container = file_path_frame, expand = TRUE, fill = TRUE)
  ...
  import_kit_info_frame <- gframe(text = strings$str_frm_new, horizontal = FALSE, spacing = 1, container = main_group)
  gene_mapperid_btn <- gbutton(text = strings$str_btn_gene_mapperid, container = import_kit_info_frame)
  gene_marker_btn <- gbutton(text = strings$str_btn_gene_marker, container = import_kit_info_frame)
  
  # Handler for importing kits
  addHandlerClicked(gene_mapperid_btn, handler = function(h, ...) {
    readGeneMapperKit_gui(
      env = env,
      savegui = savegui,
      debug = debug,
      parent = main_window,
  ...
  kit_info_layout_frame <- ggroup(horizontal = FALSE, use.scrollwindow = TRUE, expand = TRUE, fill = TRUE, container = main_group)
  
  # Layout to display the kit information.
  kit_info_layout <- glayout(container = kit_info_layout_frame, spacing = 1, expand = TRUE, fill = "both", homogeneous = TRUE)
  
  # SAVE FRAME #################################################################
  
  # Save options (Append, Overwrite)
  save_options_frame <- gframe(text = strings$str_frm_save, horizontal = FALSE, expand = FALSE, spacing = 1, container = main_group)
  save_file_btn <- gbutton(text = strings$str_btn_save, expand = TRUE, container = save_options_frame)
  ...
  read_from_gui <- function(kit_info, kit_append) {
    ...
    if (!any(short_name[!remove_kit] == "")) {
      ...
      if (!any(existing_short_names)) {
        ...
        for (p in seq_along(panels)) {
          update_kit_info(panels[p], short_name[p], full_name[p], sex_markers[p], qs_markers[p])
        }
        ...
        gmessage(
          msg = paste(strings$str_msg_duplicate1, short_name[existing_short_names][1], strings$str_msg_duplicate2),
  ...
  update_kit_info <- function(panel, short_name, full_name, sex_markers, qs_markers) {
  ...
  update_marker_flag <- function(panel, markers, flag_name) {
    ...
    if (!all(current_markers %in% kit_info$Marker[sel_panel])) {
      stop(paste("Given", flag_name, ":", paste(current_markers[!current_markers %in% kit_info$Marker[sel_panel]], collapse = ","), "not found in", panel))
    }
  }
  ...
  pad_label <- function(label, max_length) {
    pad_length <- max(0, 2 * max_length - nchar(label) + 2) # Add extra spaces for padding
  ...
  populate_gui_with_kits <- function(kit_info_input, add_kit = NULL) {
    ...
    if (!"Short.Name" %in% names(kit_info_input)) {
      ...
      kit_info_input$Short.Name <- rep("", nrow(kit_info_input)) # Add an empty string column
    }
    ...
    if (!"Full.Name" %in% names(kit_info_input)) {
      ...
      kit_info_input$Full.Name <- rep("", nrow(kit_info_input)) # Add an empty string column
    }
    ...
    if (!"Sex.Marker" %in% names(kit_info_input)) {
      ...
      kit_info_input$Sex.Marker <- rep(FALSE, nrow(kit_info_input)) # Add a FALSE bolean column
    }
    ...
    if (!"Quality.Sensor" %in% names(kit_info_input)) {
      ...
      kit_info_input$Quality.Sensor <- rep(FALSE, nrow(kit_info_input)) # Add a FALSE bolean column
    }
    ...
    str_lbl_full_name_padded <- pad_label(strings$str_lbl_full_name, max_full_name_length)
    
    # Add titles to the layout
    kit_info_layout[1, 1] <- glabel(text = strings$str_lbl_remove, container = kit_info_layout)
    kit_info_layout[1, 2] <- glabel(text = strings$str_lbl_panel, container = kit_info_layout)
    kit_info_layout[1, 3] <- glabel(text = strings$str_lbl_short_name, container = kit_info_layout)
    kit_info_layout[1, 4] <- glabel(text = str_lbl_full_name_padded, expand = TRUE, fill = "both", container = kit_info_layout)
    kit_info_layout[1, 5] <- glabel(text = strings$str_lbl_sex, expand = TRUE, fill = "both", container = kit_info_layout)
    kit_info_layout[1, 6] <- glabel(text = strings$str_lbl_sensors, expand = TRUE, fill = "both", container = kit_info_layout)
    ...
    for (p in seq_along(panels)) {
      ...
      kit_info_layout[p + 1, 1] <- gcheckbox(text = "", checked = FALSE, expand = FALSE, container = kit_info_layout)
      kit_info_layout[p + 1, 2] <- glabel(text = panels[p], expand = FALSE, container = kit_info_layout)
      kit_info_layout[p + 1, 3] <- gedit(text = short_name, expand = FALSE, container = kit_info_layout)
      kit_info_layout[p + 1, 4] <- gedit(text = full_name, expand = TRUE, fill = "both", container = kit_info_layout)
      kit_info_layout[p + 1, 5] <- gedit(text = sex_markers, expand = TRUE, fill = "both", container = kit_info_layout)
      kit_info_layout[p + 1, 6] <- gedit(text = quality_sensors, expand = TRUE, fill = "both", container = kit_info_layout)
    }
  }
