...
plotResultType_gui <- function(env = parent.frame(), savegui = NULL, debug = FALSE, parent = NULL) {
  ...
  # Default strings.
  strWinTitle <- "Plot result type"
  strChkGui <- "Save GUI settings"
  strBtnHelp <- "Help"
  strFrmDataset <- "Dataset"
  strLblDataset <- "Result type dataset:"
  ...
  strChkLabels <- "Print values as bar labels"
  ...
  strLblMainTitle <- "samples analysed"
  ...
  if (!is.null(dtStrings)) {
    ...
    strtmp <- dtStrings["strLblXTitleType"]$value
    strLblXTitleType <- ifelse(is.na(strtmp), strLblXTitleType, strtmp)

    strtmp <- dtStrings["strLblYTitleProportion"]$value
    strLblYTitleProportion <- ifelse(is.na(strtmp), strLblYTitleProportion, strtmp)
  ...
  grid4[1, 1] <- f1_print_chk <- gcheckbox(text = strChkLabels, checked = TRUE, container = grid4)
  ...
  .plot <- function() {
    ...
    if (is.data.frame(.gData)) {
      ...
      if (val_titles) {
        ...
        mainTitle <- paste(numberOfSamples, strLblMainTitle)
        xTitle <- strLblXTitleType
        if (val_prop) {
          yTitle <- strLblYTitleProportion
        } else {
